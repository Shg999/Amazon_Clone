...
const Cart = () => {
  ...
  return (
    <div className='w-full bg-gray-100 p-4'>
     {
         ...
         {/* Products start here */}
         {
          products.map((item)=>(
              ...
              gap-6'>
                  ...
                  <div className='w-1/5'>
                      <img className='w-full h-44 object-contain' src={item.image}/>
                  </div>
                  </div>
                  <div className='w-3/5'>
                      ...
                      <div className='bg-[#F0F2F2] flex justify-center items-center gap-1 w-24 py-1
                      text-center drop-shadow-lg rounded-md'>
                        <p>Qty:</p>
                        <p onClick={()=>dispatch(decrementQuantity(item.id))} className='bg-gray-200 cursor-pointer px-1 rounded-md hover:bg-gray-400 duration-300'>-</p>
                        <p>{item.quantity}</p>
                        <p onClick={()=>dispatch(incrementQuantity(item.id))} className='bg-gray-200 cursor-pointer px-1 rounded-md hover:bg-gray-400 duration-300'>+</p>
                      </div>
                      <button 
                      onClick={()=>dispatch(deleteItem(item.id))}
                      className='bg-red-500 w-36 py-1 rounded-lg text-white mt-2
                      hover:bg-red-700 active:bg-red-900 duration-300'>Remove</button>
                  </div>
                  <div>
      ...
      <div className='w-full h-52 flex flex-col items-center justify-center p-4 bg-white col-span-1'>
      <div>
        <p className='flex gap-2 items-start text-sm'><span>
          <CheckCircleIcon className='bg-white text-green-500 rounded-md'/>
          </span>{" "}
      ...
      from-yellow-400 to-yellow-200 border hover:from-yellow-300 hover:to-yellow-200
    ...
    className='item-center justify-center flex gap-4 py-10'>
     <div>
      <img className='w-80 rounded-lg p-4 mx-auto' src={emptyCart} />
     </div>
